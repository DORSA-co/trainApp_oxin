...
def SSI(img, block_size="Medium", defect_th=0, noise_th=7, noise=True, heatmap=False):
    ...
    if heatmap:
        hm = np.concatenate(
            [t[0]["heatmap"], t[1]["heatmap"], t[2]["heatmap"], t[3]["heatmap"]]
        )
...
def FindDefectiveBlocks(
    gray, block_size="Medium", defect_th=0, noise_th=7, noise=True, heatmap=False
):
...
def CreateHeatmap(gray, img):
    ...
    t = cv2.normalize(
        (imgt * gray).astype("uint8"), None, 0, 255, cv2.NORM_MINMAX, -1, None
    )
...
if __name__ == "__main__":
    ...
    jsonMainPath = (
        "/home/reyhane/PycharmProjects/trainApp_oxin8/oxin_image_grabber_imgProcessing"
    )
    img_format = ".png"
    json_format = ".json"
    for s in ["BOTTOM", "TOP"]:
        for c in range(1, 13):
            ...
            for f in range(1, 200):
                ...
                df = SSI_2(
                    img,
                    path=os.path.join(
                        jsonMainPath, sheetID, s, str(c), str(f) + json_format
                    ),
                    block_size="Medium",
                    defect_th=0,
                    noise_th=7,
