...
def train(
    hyp, opt, device, callbacks, my_callback
    ...
    for epoch in range(
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, map50s, _ = validate.run(
                    data_dict,
            ...
            my_callback.on_epoch_end(
                epoch, dict(zip(loggers.keys + loggers.class_name_keys, log_vals))
            )
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if opt.s3_upload_dir:
                    ...
                    s3_file = "s3://" + str(
                        Path(opt.s3_upload_dir.replace("s3://", ""))
                        / save_dir.name
                        / "weights"
                        / "best.pt"
                    ).replace(os.sep, "/")
                    LOGGER.info(
                        f"{colorstr('aws:')} Uploading best weight to AWS S3..."
                    )
                    result = upload_file_to_s3(local_file=str(best), s3_file=s3_file)
                    if result:
                        LOGGER.info(
                            f"{colorstr('aws:')} Best weight has been successfully uploaded to {s3_file}"
                        )
    ...
    log_vals = list(best_mloss) + list(results) + lr
    results = dict(zip(loggers.keys, log_vals))
    return results
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--mmdet_tags",
        action="store_true",
        help="Log train/val keys in MMDetection format",
    )
    parser.add_argument("--callback", default=None, help="Custom callback object")
    ...
    parser.add_argument(
        "--neptune_token", type=str, default=None, help="neptune.ai api token"
    )
    parser.add_argument(
        "--neptune_project",
        type=str,
        default=None,
        help="https://docs.neptune.ai/api-reference/neptune",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="aws s3 folder directory to upload best weight and dataset",
    )
    parser.add_argument(
        "--upload_dataset", action="store_true", help="upload dataset to aws s3"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="huggingface.co model_id to be uploaded",
    )
    parser.add_argument(
        "--hf_token",
        type=str,
        default=None,
        help="huggingface.co token to be used for upload",
    )
    parser.add_argument(
        "--hf_private", action="store_true", help="upload model to private repo"
    )
    parser.add_argument(
        "--hf_dataset_id",
        type=str,
        default=None,
        help="huggingface dataset id to link the model",
    )

    # Roboflow arguments
    parser.add_argument(
        "--roboflow_token", type=str, default=None, help="roboflow api token"
    )

    return parser.parse_known_args()[0] if known else parser.parse_args()


def main(opt, callbacks=Callbacks(), my_callback=None):
    # Checks
    if RANK in {-1, 0}:
        print_args(vars(opt))
        # check_git_status()
    ...
    if not opt.evolve:
        results = train(
            opt.hyp, opt, device, callbacks=callbacks, my_callback=my_callback
        )
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, device, callbacks, my_callback)
            callbacks = Callbacks()
...
def run(my_callback=None, **kwargs):
    # Usage: import train; train.run(data='coco128.yaml', imgsz=320, weights='yolov5m.pt')
    opt = parse_opt(True)
    for k, v in kwargs.items():
        setattr(opt, k, v)
    results = main(opt, my_callback=my_callback)
    return results
