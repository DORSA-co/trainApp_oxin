...
def train(hyp, opt, device, callbacks, my_callback):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            my_callback.on_epoch_end(epoch, dict(zip(loggers.keys + loggers.class_name_keys, log_vals)))
...
def main(opt, callbacks=Callbacks(), my_callback=None):
    ...
    # Train
    if not opt.evolve:
        results = train(opt.hyp, opt, device, callbacks=callbacks, my_callback=my_callback)
