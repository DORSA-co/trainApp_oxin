...
def train_binary(binary_algorithm_name, binary_input_size, binary_input_type, binary_epoch, binary_batch, binary_lr, binary_te, binary_vs, binary_dp, weights_path, api_obj):
    ...
    return create_binary_model_record_dict([binary_algorithm_name,
...
def train_localization(loc_algorithm_name, loc_input_size, loc_input_type, loc_epoch, loc_batch, loc_lr, loc_vs, loc_dp, weights_path, api_obj):
    ...
    try:
        history = model.fit(trainGen,
                steps_per_epoch=int(train_data_count/loc_batch) + 1,
                epochs=loc_epoch,
                callbacks=[my_callback ],
                validation_data=testGen,
                validation_steps=test_data_count//loc_batch + 1, 
                initial_epoch=0)
        api_obj.ui.logger.create_new_log(message=texts.MESSEGES['FIT_MODEL']['en'])
    except Exception as e:
        api_obj.ui.logger.create_new_log(message=texts.ERRORS['FIT_MODEL_FAILED']['en'], level=5)
