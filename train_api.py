...
def train_binary(
    ...
    try:
        ...
        return (
            False,
            (texts.ERRORS["SET_PROCESSOR_FAILED"][api_obj.language], "train", 3),
        )

    # Create weights path
    weights_path = os.path.join(
        weights_path, date_funcs.get_datetime(persian=SHAMSI_DATE, folder_path=True)
    )
    try:
        ...
        return (
            False,
            (texts.ERRORS["CREATE_BWPATH_FAILED"][api_obj.language], "train", 3),
        )
    ...
    vspe = (
        ...
        else (testGen.n // binary_batch)
    )

    if spe < 1 or vspe < 1:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["low_data"]["en"], level=5
        )
    ...
    if binary_te > 0:
        ...
        if binary_algorithm_name == ALGORITHM_NAMES["binary"][0]:
            try:
                model = models.xception_cnn(
                    ...
                    fine_tune_layer=120,
                    weights=os.path.join(weights_path, 'epoch_{}'.format(binary_epoch)) if binary_epoch>0 else binary_pretrain_path,
                )
                api_obj.ui.logger.create_new_log(
                ...
                return (
                    False,
                    (
                        texts.ERRORS["CREATE_FTMODEL_FAILED"][api_obj.language].format(
                            binary_algorithm_name
                        ),
                        "train",
                        3,
        ...
        try:
            ...
            return (
                False,
                (texts.ERRORS["FIT_FTMODEL_FAILED"][api_obj.language], "train", 3),
            )
        ...
        try:
            ...
            return (
                False,
                (texts.ERRORS["SAVE_FTBMODEL_FAILED"][api_obj.language], "train", 3),
            )
    ...
    return (
        True,
        create_binary_model_record_dict(
            [
                binary_algorithm_name,
...
def train_localization(
    ...
    try:
        ...
        return (
            False,
            (texts.ERRORS["SET_PROCESSOR_FAILED"][api_obj.language], "l_train", 3),
        )
    ...
    try:
        if not os.path.exists(weights_path):
            os.makedirs(weights_path)
        api_obj.ui.logger.create_new_log(
            message=texts.MESSEGES["CREATE_LWPATH"]["en"] + weights_path
        )
    except Exception as e:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["CREATE_LWPATH_FAILED"]["en"] + weights_path, level=5
        )
        return (
            False,
            (texts.ERRORS["CREATE_LWPATH_FAILED"][api_obj.language], "l_train", 3),
        )
    ...
    try:
        if not loc_input_type:
            ...
            (
                ...
                paths=loc_dp,
                api_obj=api_obj,
                img_folder="image_splitted",
                label_folder="label_splitted",
                mulit_mask_class=False,
                split=loc_vs,
            )
        api_obj.ui.logger.create_new_log(
            message=texts.MESSEGES["dataset_splitted"]["en"]
        )
    except Exception as e:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["dataset_splitted_failed"]["en"], level=5
        )
        return (
            False,
            (texts.ERRORS["dataset_splitted_failed"][api_obj.language], "l_train", 3),
        )
    ...
    try:
        ...
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["CREATE_LOC_GEN_FAILED"]["en"], level=5
        )
        return (
            False,
            (texts.ERRORS["CREATE_LOC_GEN_FAILED"][api_obj.language], "l_train", 3),
        )
    ...
    try:
        if loc_algorithm_name == ALGORITHM_NAMES["localization"][0]:
            ...
            model = models.unet_model(
                ...
                num_class=1,
                mode=models.BINARY,
                weights_path=loc_pretrain_path,
            )
        api_obj.ui.logger.create_new_log(
            message=texts.MESSEGES["CREATE_MODEL"]["en"].format(loc_algorithm_name)
        )
    except Exception as e:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["CREATE_MODEL_FAILED"]["en"].format(
    ...
    try:
        my_callback = callbacks.CustomCallback(
            None, model_type="localization", api_obj=api_obj
        )
        ...
        return (
            False,
            (texts.ERRORS["CALLBACK_CREATE_FAILED"][api_obj.language], "l_train", 3),
        )

    spe = (
        (train_data_count // loc_batch + 1)
        if ((train_data_count // loc_batch) != (train_data_count / loc_batch))
        else (train_data_count // loc_batch)
    )
    vspe = (
        (test_data_count // loc_batch + 1)
        if ((test_data_count // loc_batch) != (test_data_count / loc_batch))
        else (test_data_count // loc_batch)
    )
    ...
    try:
        ...
        return (
            False,
            (texts.ERRORS["FIT_MODEL_FAILED"][api_obj.language], "l_train", 3),
        )
    ...
    if os.path.exists(os.path.join(weights_path, "localization_model.h5")):
        ...
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["SAVE_LMODEL_FAILED"]["en"], level=3
        )
        return (
            False,
            (texts.ERRORS["SAVE_LMODEL_FAILED"][api_obj.language], "l_train", 3),
        )
    ...
    return (
        True,
        create_localization_model_record_dict(
            [
                loc_algorithm_name,
                loc_pretrain_path,
...
def train_yolo(
    ...
    try:
        if not y_input_type:
            train_images_path, val_images_path = split_yolo_dataset(
                paths=y_dp,
                api_obj=api_obj,
                img_folder="image",
        ...
        return (
            False,
            (texts.ERRORS["dataset_splitted_failed"][api_obj.language], "y_train", 3),
        )

    try:
        path = dataGenerator.create_yolo_yaml_file(
            train_images_path, val_images_path, class_name_to_id_mapping
        )
        api_obj.ui.logger.create_new_log(message=texts.MESSEGES["CREATE_Y_GEN"]["en"])
    except Exception as e:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["CREATE_Y_GEN_FAILED"]["en"], level=5
        )
        return (
            False,
            (texts.ERRORS["CREATE_Y_GEN_FAILED"][api_obj.language], "y_train", 3),
        )
    ...
    try:
        ...
        results = run(
            ...
            weights="models/localization_and_classification/yolov{}.pt".format(
                y_algorithm_name
            ),
            # weights = '',
        ...
        return (
            False,
            (texts.ERRORS["FIT_MODEL_FAILED"][api_obj.language], "y_train", 3),
        )

    # Save weights
    if os.path.exists(os.path.join(weights_path, date, "best.pt")) and os.path.exists(
        os.path.join(weights_path, date, "last.pt")
    ):
        api_obj.ui.logger.create_new_log(message=texts.MESSEGES["SAVE_YMODEL"]["en"])
    else:
        api_obj.ui.logger.create_new_log(
            message=texts.ERRORS["SAVE_YMODEL_FAILED"]["en"], level=3
        )
        return (
            False,
            (texts.ERRORS["SAVE_YMODEL_FAILED"][api_obj.language], "y_train", 3),
        )
    ...
    return (
        ...
        create_yolo_model_record_dict(
            [
                ...
                results["metrics/mAP_0.5"],
                results["metrics/mAP_0.5:0.95"],
