...
def get_localization_models_from_db(
    db_obj, count=False, limit_size=localization_table_nrows, offset=0
):
    ...
    res, lmodels_list = db_obj.get_localization_models(
...
def set_lmodels_on_ui_tabel(ui_obj, lmodels_list):
    ...
    ui_obj.localization_history_tabel.setHorizontalHeaderLabels(
        localization_headers if ui_obj.language == "en" else localization_headers_fa
    )
    ...
    for row_idx, lmodel in enumerate(lmodels_list):
        for col_idx in range(localization_table_ncols):
            ...
            if col_idx == 1:
                lmodel[localization_headers_db[col_idx]] = (
                    "-"
                    if lmodel[localization_headers_db[col_idx]] == ""
                    else lmodel[localization_headers_db[col_idx]]
                )
            if col_idx == 3:
                lmodel[localization_headers_db[col_idx]] = (
                    "Split"
                    if lmodel[localization_headers_db[col_idx]] == "1"
                    else "Resize"
                )
            table_item = sQTableWidgetItem(
                str(lmodel[localization_headers_db[col_idx]])
            )
...
def get_localization_model_filter_info_from_ui(ui_obj):
    ...
    try:
        # get params from UI
        lmodel_info = {}
        lmodel_info["algo_name"] = [
            translate_localization_algorithm_id_to_name(
                algo_id=ui_obj.localization_name_filter_combo.currentText(),
                reverse=True,
            )
        ]
        lmodel_info["epochs"] = [
            ui_obj.localization_epoch_min_filter_lineedit.text(),
            ui_obj.localization_epoch_max_filter_lineedit.text(),
        ]
        lmodel_info["batch_size"] = [
            ui_obj.localization_batch_min_filter_lineedit.text(),
            ui_obj.localization_batch_max_filter_lineedit.text(),
        ]
        lmodel_info["split_ratio"] = [
            ui_obj.localization_split_min_filter_lineedit.text(),
            ui_obj.localization_split_max_filter_lineedit.text(),
        ]
        lmodel_info["val_loss"] = [
            ui_obj.localization_loss_min_filter_lineedit.text(),
            ui_obj.localization_loss_max_filter_lineedit.text(),
        ]
        lmodel_info["val_accuracy"] = [
            ui_obj.localization_acc_min_filter_lineedit.text(),
            ui_obj.localization_acc_max_filter_lineedit.text(),
        ]
        lmodel_info["val_iou"] = [
            ui_obj.localization_iou_min_filter_lineedit_2.text(),
            ui_obj.localization_iou_max_filter_lineedit_2.text(),
        ]
        lmodel_info["val_fscore"] = [
            ui_obj.localization_fscore_min_filter_lineedit.text(),
            ui_obj.localization_fscore_max_filter_lineedit.text(),
        ]

        # date
        lmodel_info["start_date"] = [
            ui_obj.localization_start_year_lineedit.text(),
            ui_obj.localization_start_month_lineedit.text(),
            ui_obj.localization_start_day_lineedit.text(),
        ]
        lmodel_info["end_date"] = [
            ui_obj.localization_end_year_lineedit.text(),
            ui_obj.localization_end_month_lineedit.text(),
            ui_obj.localization_end_day_lineedit.text(),
        ]
        ...
        ui_obj.logger.create_new_log(
            message=texts.ERRORS["ui_get_lmodel_filter_params_failed"]["en"], level=5
        )
        return []


# get users from database
def get_filtered_localization_models_from_db(
    ui_obj,
    db_obj,
    filter_params,
    limit_size=localization_table_nrows,
    offset=0,
    count=False,
):
    ...
    try:
        if (
            filter_params["val_iou"][0] != ""
            and filter_params["val_iou"][1] != ""
            and float(filter_params["val_iou"][0]) > float(filter_params["val_iou"][1])
        ):
            ui_obj.set_warning(
                texts.ERRORS["IOU_RANGE_INCORRECT"][ui_obj.language],
                "localization_model_history",
                level=3,
            )
            return "error", []
        elif bool(filter_params["val_iou"][0] == "") ^ bool(
            filter_params["val_iou"][1] == ""
        ):
            ui_obj.set_warning(
                texts.WARNINGS["IOU_RANGE_EMPTY"][ui_obj.language],
                "localization_model_history",
                level=2,
            )
            return "error", []
            ...
            params.append(
                [
                    str(float(filter_params["val_iou"][0])),
                    str(float(filter_params["val_iou"][1])),
                ]
            )
            cols.append("val_iou")
    except:
        ui_obj.set_warning(
            texts.ERRORS["IOU_FORMAT_INVALID"][ui_obj.language],
            "localization_model_history",
            level=3,
        )
        return "error", []
    ...
    try:
        if (
            filter_params["val_fscore"][0] != ""
            and filter_params["val_fscore"][1] != ""
            and float(filter_params["val_fscore"][0])
            > float(filter_params["val_fscore"][1])
        ):
            ui_obj.set_warning(
            ...
            return "error", []
        elif bool(filter_params["val_fscore"][0] == "") ^ bool(
            filter_params["val_fscore"][1] == ""
        ):
            ui_obj.set_warning(
            ...
            params.append(
                [
                    str(float(filter_params["val_fscore"][0])),
                    str(float(filter_params["val_fscore"][1])),
                ]
            )
    ...
    # get from db
    try:
        res, defects_list = db_obj.search_localization_model_by_filter(
            parms=params,
            cols=cols,
            limit=True,
        ...
        if res:
            ...
            ui_obj.notif_manager.append_new_notif(
                message=texts.ERRORS["database_get_lmodels_failed"][ui_obj.language],
                level=4,
            )
            return "error", []
...
class Localization_model_train_worker(sQObject):
    ...
    def train_model(self):
        ...
        if not lmodel_records[0]:
            self.warning.emit(
                lmodel_records[1][0], lmodel_records[1][1], None, lmodel_records[1][2]
            )
        else:
            ...
            if lmodel_records:
                ...
                self.ui_obj.notif_manager.append_new_notif(
                    message=texts.MESSEGES["lmodel_trained"][self.ui_obj.language],
                    level=1,
                )
                ...
                if self.api_obj.lmodel_train_result:
                    ...
                    self.warning.emit(
                        texts.ERRORS["database_add_lmodel_failed"][
    ...
    def save_l_model(self, model, path, epoch):
        try:
            ...
            self.ui_obj.logger.create_new_log(
                message=texts.MESSEGES["SAVE_LMODEL_EPOCH"]["en"].format(epoch)
            )
        except:
            self.ui_obj.logger.create_new_log(
                message=texts.ERRORS["SAVE_LMODEL_EPOCH_FAILED"]["en"].format(epoch),
                level=5,
            )
            self.warning.emit(
                texts.ERRORS["SAVE_LMODEL_EPOCH_FAILED"][self.api_obj.language].format(
                    epoch
                ),
                "l_train",
                None,
